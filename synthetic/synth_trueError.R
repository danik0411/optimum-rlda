estimateTrueError = function( X0 , X1 , sigma , C , means , xs , alphas , p , kappa , gammaValues , checkValue ) {
...
for ( i in 1:length( gammaValues ) ) {
    ...
    myD = t( diff ) %*% H %*% sigma %*% H %*% ( diff );

    for ( j in 1:2 ) {
    	mu = as.matrix( means[,j] , ncol=1 );
    	myG = t( mu - (x0+x1)/2 ) %*% H %*% diff;
