...
uns = read.delim(file="BhattacherjeeLung2001_12600x203.txt", header = FALSE, sep = ",", nrows=1)
un0=as.numeric(uns[1]) #first n0 samples are from one class
un1=as.numeric(uns[2]) #first n1 samples are from one class
totalData = read.delim(file="BhattacherjeeLung2001_12600x203.txt", header = FALSE, sep = "\t", skip=2);
...
print(class(totalData));
totalData = totalData[,c(1:203)];	# the last term of totalData is always NA, thus remove it
...
save( list = c("sampleNumbers" , "Data") , file="bhatta_p50.RData" );
