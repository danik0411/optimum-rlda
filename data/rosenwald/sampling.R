...
uns = read.delim(file="Rosenwald2002_5013x203.txt", header = FALSE, sep = ",", nrows=1)
un0=as.numeric(uns[1]) #first n0 samples are from one class
un1=as.numeric(uns[2]) #first n1 samples are from one class
totalData = read.delim(file="Rosenwald2002_5013x203.txt", header = FALSE, sep = "\t", skip=2);
...
save( list = c("sampleNumbers" , "Data") , file="rosenwald_p150.RData" );
