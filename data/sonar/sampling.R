...
uns = read.delim(file="sonar.txt", header = FALSE, sep = ",", nrows=1)
un0=as.numeric(uns[1]) #first n0 samples are from one class
un1=as.numeric(uns[2]) #first n1 samples are from one class
totalData = read.delim(file="sonar.txt", header = FALSE, sep = "\t", skip=2);
...
save( list = c("sampleNumbers" , "Data") , file="sonar.RData" );
