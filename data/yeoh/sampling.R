...
uns = read.delim(file="YeohLeukemia2002_5077x248.txt", header = FALSE, sep = ",", nrows=1)
un0=as.numeric(uns[1]) #first n0 samples are from one class
un1=as.numeric(uns[2]) #first n1 samples are from one class
totalData = read.delim(file="YeohLeukemia2002_5077x248.txt", header = FALSE, sep = "\t", skip=2);
...
print(class(totalData));
totalData = totalData[,-249];	# the last term of totalData is always NA, thus remove it
...
save( list = c("sampleNumbers" , "Data") , file="yeoh_p150.RData" );1
